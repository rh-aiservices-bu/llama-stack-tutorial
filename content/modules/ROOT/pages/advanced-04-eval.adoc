...
You learned Llama Stack evaluation fundamentals:

* **subset_of**: Fast exact matching, but case-sensitive
* **llm_as_judge**: Flexible semantic evaluation with custom prompts
* **Dataset evaluation**: Testing on real knowledge benchmarks

=== Key Takeaways

* Use subset_of for exact format requirements
* Use llm_as_judge for semantic similarity evaluation  
* Real datasets reveal model knowledge limitations
* Always test before production deployment

The evaluation framework helps ensure quality before deploying AI applications.